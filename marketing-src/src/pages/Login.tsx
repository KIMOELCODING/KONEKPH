    ...
    <div className="login-shell">
      <form className="login-card" onSubmit={submit}>
        <div className="brand">
          ...
          <div>
            <h1>ProList <span style={{ color: 'var(--tl)' }}>.marketing</span></h1>
          </div>
        </div>
        <p className="sub">Sign in with your marketing team account.</p>
        {err && <div className="alert alert-error"><i className="fa-solid fa-circle-exclamation"></i> {err}</div>}
        <div className="field">
          <label>Email</label>
          <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="you@prolist.ph" autoComplete="email" />
        </div>
        <div className="field">
